    ...
    <div className="space-y-24">
          ...
          <div className="grid gap-12 lg:grid-cols-2 lg:items-center">
            ...
            <div className="space-y-8">
              ...
              <div className="flex flex-col gap-4 sm:flex-row">
                <Link href="/katalog">
                  <Button size="lg" className="bg-[#D92820] text-white hover:bg-[#D92820]/90 rounded-xl px-8">
                    Jelajahi Katalog
            ...
            <div className="relative">
              {banner ? (
                <div className="relative aspect-[4/3] overflow-hidden rounded-2xl shadow-2xl">
                  <Image src={banner.image} alt={banner.title} fill className="object-cover" priority sizes="(max-width: 1024px) 100vw, 50vw" />
                </div>
              ) : (
                <div className="flex aspect-[4/3] items-center justify-center rounded-2xl bg-[#111111] shadow-2xl">
                  <div className="text-center space-y-4 px-8">
                    ...
                    <p className="text-sm text-white/60">Banner akan muncul di sini</p>
                  </div>
                </div>
              )}
            </div>
        ...
        <section className="bg-[#F5F5F5]">
          <div className="mx-auto max-w-7xl px-4 py-20 sm:px-6">
            <div className="text-center space-y-3 mb-12">
              <h2 className="text-3xl font-bold text-[#111111] sm:text-4xl font-heading">Keunggulan Rah-MART</h2>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="flex items-center justify-between mb-10">
          <h2 className="text-3xl font-bold text-[#111111] font-heading">Produk Terbaru</h2>
          <Link href="/katalog">
        ...
        {products.length === 0 ? (
          <p className="py-16 text-center text-[#111111]/60">Belum ada produk tersedia.</p>
        ) : (
              ...
              <Link key={product.id} href={`/produk/${product.id}`}>
                <Card className="group overflow-hidden border-[#111111]/5 bg-white transition-all hover:shadow-lg">
                  <div className="relative aspect-[4/3] bg-[#F5F5F5] overflow-hidden">
                    <Image src={product.mainImage} alt={product.name} fill className="object-cover transition-transform group-hover:scale-105" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" />
                  </div>
                  <CardContent className="space-y-3 p-5">
                    <h3 className="font-semibold text-[#111111] line-clamp-1 font-heading">{product.name}</h3>
                    <p className="text-lg font-bold text-[#D92820]">{formatPrice(Number(product.price))}</p>
                    <p className="text-sm text-[#111111]/60 line-clamp-1">{product.location}</p>
                  </CardContent>
                </Card>
              </Link>
            ))}
      ...
      <section className="bg-[#111111]">
        <div className="mx-auto max-w-7xl px-4 py-20 text-center sm:px-6">
          <h2 className="text-3xl font-bold text-white sm:text-4xl font-heading">Butuh Konsultasi?</h2>
          <p className="mt-4 text-white/60">Hubungi kami via WhatsApp untuk informasi lebih lanjut.</p>
          <Link href="https://wa.me/6289643483985" target="_blank">
            <Button size="lg" className="mt-8 bg-[#D92820] text-white hover:bg-[#D92820]/90 rounded-xl px-10">
              Chat via WhatsApp
            </Button>
          </Link>
        </div>
